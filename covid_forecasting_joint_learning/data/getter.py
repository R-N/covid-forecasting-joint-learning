...
class DataGetter:
    ...
    def get_data(self, date):
        ...
        data = resp.json()
        data = [{'kabko':x['kabko'], 'date':date, 'infected_total':int(x['confirm']), 'recovered':int(x['confirm_selesai']), 'dead':int(x['meninggal_rtpcr'])} for x in data]
        data = [{**x, 'infected':(x['infected_total']-x['recovered']-x['dead'])} for x in data]
        return data

    def get_data_bulk(self, dates, max_process_count=366, max_tasks_per_child=100, pool=None):
