...
def main_1(data_path, labeled_dates=DataCol.LABELED_DATES, cols=DataCol.COLS):
    ...
    loader = preprocessing_0(loader)

    kabkos = get_kabkos(loader)
    # kabko_indices = {kabkos[i].name: i for i in range(len(kabkos))}
    kabkos = preprocessing_1(kabkos)

    for kabko in kabkos:
        kabko.data = kabko.add_dates(kabko.data, dates=labeled_dates)[cols]

    groups = preprocessing_2(kabkos)
    # groups = [groups[2]]
    for group in groups:
        preprocessing_3(group.members)

    n_clusters = DEFAULT_GROUPS_N_CLUSTERS
