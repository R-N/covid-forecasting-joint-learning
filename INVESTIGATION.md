...
1. **Member batching via HFTA rather than by hand.** The largest available speedup, and it exists as a
   ...
   **Partially applied**: `hfta`'s ops (`hfta/ops/linear.py`, `conv.py`) turned out to be plain `torch.bmm`/grouped-conv under the hood, not CUDA-only kernels -- installed and numerically verified for real (`pip install git+https://github.com/UofT-EcoSystem/hfta`, CPU torch, out-of-band venv): 4 architecturally-identical `nn.Linear` layers fused via `hfta.ops.Linear(B=4)` produced output identical (max abs diff `0.0`) to running them separately. `model/modules/fused.py` wraps this (`fused_linear_from_members`), added as an optional dependency in `requirements-experiment.txt` (pinned commit), `tests/test_fused_linear.py` (stub reproduces `hfta.ops.Linear` verbatim, MIT-licensed, since `hfta` isn't installed dev-side -- the out-of-band reproduction command is in the test's docstring). **Not done**: `hfta` has no fused LSTM op, so the recurrent `LILSTMCell2`/`nn.LSTM` decoder cells stay per-member; and this isn't wired into `ClusterModel`'s training loop yet -- replacing the per-member Python loop in `model/train.py`'s `__eval` with one batched forward call across all members is a separate, larger architectural change. Training-throughput benefit (the actual point of HFTA) is still unmeasured -- needs a CUDA GPU, still absent here.
...
6. **External generalisation check on EpiCastBench.** The only way to show a method generalises beyond
   ...
   **Partially applied**: EpiCastBench's data and code are genuinely public and reachable without any API key (`https://github.com/aimltsf/EpiCastBench`, `https://www.kaggle.com/datasets/aimltsf/epicastbench`, downloads over plain HTTPS). `scripts/epicastbench_check.py` downloads it, follows EpiCastBench's own protocol (`INPUT_CHUNK=24`, `OUTPUT_CHUNK=12`, fixed-origin split from `Code/config.py`), and runs this repo's actual `NaiveModel`/`LinearModel`/`ThetaModel`/`GBTModel` (`model/comparison/`) against 3 real series (COVID India/Delhi, COVID US/California, dengue Brazil/Bahia), scored with this repo's own `loss_common.rmsse`. Real output: naive/linear/theta/gbt RMSSE of {7.77, 0.70, 4.18, 7.81} (Delhi), {0.61, 0.54, 0.19, 0.15} (California), {20.2, 20.8, 20.4, 20.1} (Bahia) -- mixed as expected (dengue's seasonal structure clearly beats all 4 short-history baselines equally badly). **Not done**: this validates the comparison-baseline code path generalizes, not the full `SingleModel` joint-learning architecture (needs the SIRD R/D breakdown and exogenous calendar features EpiCastBench's plain case-count CSVs don't provide) or the full 40-dataset/15-model sweep (needs EpiCastBench's own GPU-hungry deep baselines and a real compute budget, absent here). Not a claim of state-of-the-art performance on EpiCastBench -- a proof the code runs correctly on genuinely external data.
