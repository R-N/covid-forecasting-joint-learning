...
6/6 have real progress at the code level (same caveat as Quick wins: syntax-checked and unit/equivalence-tested, not run against real GPU-scale training). 4 are fully applied; the other 2 turned out to be more tractable than first assessed -- initially marked "blocked" on a too-quick read of the environment's constraints, corrected after actually checking network/library/data access.
...
1. **Member batching via HFTA rather than by hand.** The largest available speedup, and it exists as a
   ...
   **Partially applied, substantially deeper than the first pass**: `hfta`'s ops (`hfta/ops/linear.py`, `conv.py`) turned out to be plain `torch.bmm`/grouped-conv under the hood, not CUDA-only kernels -- installed and numerically verified for real (`pip install git+https://github.com/UofT-EcoSystem/hfta`, CPU torch, out-of-band venv). `model/modules/fused.py` now fuses the *entire* Linear-only "direct decoder" stage -- `nn.Linear` (`fused_linear_from_members`), `ResidualFC` at depth<=1 including the `ResidualBlock`/highway case (`fused_residual_fc_from_members`), `CombineHead` (`fused_combine_head_from_members`), `DirectFutureHead` (`fused_direct_future_head_from_members`), glued by `fuse_direct_decoder_from_members` -- not just a standalone `nn.Linear` microbenchmark. End-to-end verified against real `SingleModel` instances (private-only and private+shared configs) both out-of-band (real hfta, CPU torch: max abs diff `1.19e-07`/`8.94e-08`) and dev-side (`tests/test_fused_linear.py`, hfta stub reproduces `hfta.ops.Linear` verbatim). Added as an optional pinned-commit dependency in `requirements-experiment.txt`.
   **Real bug found and fixed along the way**: `ClusterModel`'s shared-branch wiring (`general.py`) only ever covered the recurrent-decoder path (`shared_head_future_cell`/`private_head_future_cell`) -- `direct_multi_horizon=True` models' `direct_shared_head`/`direct_private_head` weren't in that list at all, so every cluster member silently built its *own independent* "shared" branch instead of actually sharing it, defeating joint learning for direct-decoder models entirely. Fixed: `SingleModel.__init__` now accepts pre-built `direct_private_head`/`direct_shared_head` (mirroring `shared_head_future_cell`'s object-or-dict pattern), `ClusterModel.__init__` wires them the same way it wires everything else. `tests/test_direct_head_sharing.py` proves the shared object is genuinely the same instance across members and that a gradient computed through one member's forward pass reaches it.
   **Still not done**: `hfta` has no fused LSTM op, so `PastModel`'s `PastHead` (`nn.LSTM`) and the `LILSTMCell2`-looped recursive decoder stay per-member regardless of `direct_multi_horizon`. The fusion built here is a snapshot -- `snatch_parameters` *copies* weights into the fused op's own tensors, so it proves forward-pass correctness but isn't yet safe to train through directly (an optimizer step on the fused copy wouldn't update the original per-member layers, or vice versa). Wiring this into `ClusterModel`'s actual training loop for live training needs the fused op to be the *sole* owner of the weights (replacing, not periodically re-copying from, each member's own layer) -- a further architectural step, not attempted here. Training-throughput benefit (the actual point of HFTA) remains unmeasured -- still no CUDA GPU in this environment.
...
6. **External generalisation check on EpiCastBench.** The only way to show a method generalises beyond
   ...
   **Deepened**: `scripts/epicastbench_check.py` downloads it, follows EpiCastBench's own protocol (`INPUT_CHUNK=24`, `OUTPUT_CHUNK=12`, fixed-origin split from `Code/config.py`), and runs this repo's actual `NaiveModel`/`LinearModel`/`ThetaModel`/`GBTModel` (`model/comparison/`) against 3 real series (COVID India/Delhi, COVID US/California, dengue Brazil/Bahia), scored with this repo's own `loss_common.rmsse`. Real output: naive/linear/theta/gbt RMSSE of `{7.77, 0.70, 4.18, 7.81}` (Delhi), `{0.61, 0.54, 0.19, 0.15}` (California), `{20.2, 20.8, 20.4, 20.1}` (Bahia).
   Went further: `scripts/epicastbench_joint_learning_check.py` actually trains `SingleModel`'s real private+shared architecture on real EpiCastBench data (5 large Indian states from `covid_india.csv` as cluster members, daily new confirmed cases -- output_size=1, no SIRD reconstruction, since EpiCastBench has no R/D breakdown to reconstruct against; this tests the joint-learning *architecture*, not the SIRD domain model, which honestly doesn't apply to data this shape). Shared branch built the same way `ClusterModel` now does (see the bugfix below). Trained 150 epochs, Adam, CPU, ~14 minutes real wall time. Real held-out forecast RMSSE, same target/split/metric for every arm:
   | State | naive | linear | theta | gbt | joint-learning `SingleModel` |
   |---|---|---|---|---|---|
   | Delhi | 0.76 | 1.62 | 1.38 | 1.55 | 1.91 |
   | Maharashtra | 0.46 | 0.34 | 0.40 | 0.40 | 3.66 |
   | Karnataka | 0.41 | 1.51 | 0.87 | 0.75 | 3.14 |
   | Tamil Nadu | 3.75 | 4.08 | 0.60 | 3.79 | 5.18 |
   | Kerala | 0.68 | 1.57 | 1.04 | 0.65 | 4.38 |

   (`gbt`'s `GradientBoostingRegressor` isn't seeded, so its column varies by a few hundredths run to run; every other column, and the joint-learning column, is exactly reproducible.)

   Honest result: the untuned joint-learning model loses to every baseline on every state here. Consistent with, not contrary to, this doc's own central finding (US COVID-19 Forecast Hub: only 7/22 teams beat naive) -- and expected at this scale: 5 members, 150 epochs, no Optuna search, no hyperparameter tuning, vs. baselines that need none. This is a smoke test that the architecture trains and forecasts correctly on genuinely external multi-region data, not a claim it's competitive without the tuning budget the real pipeline gives it.
   **Not done**: the full 40-dataset/15-model sweep (needs EpiCastBench's own GPU-hungry deep baselines -- Transformer, Chronos2, TimesFM, DeepAR, NBeats, NHiTS, TCN, TiDE, KAN -- and real compute budget, absent here; EpiCastBench's own paper already reports those numbers) or an Optuna-tuned run of `SingleModel` (would need real GPU time to be worth running at that budget). Not a claim of state-of-the-art performance on EpiCastBench -- a proof the code, including the actual joint-learning mechanism, runs correctly end-to-end on genuinely external data.
