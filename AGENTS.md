...
- `loss_common.py::naive()` scales `msse`/`rmsse` against first differences (`step=1`). The convention for daily series is the seasonal naive at lag 7, and these series carry a strong weekly reporting cycle, so the current denominator is inflated and flatters every model. Decide this deliberately before trusting any scaled-error number.
- Generic PyTorch tuning advice does not transfer here: whole splits are already CUDA tensors so `num_workers`/`pin_memory` are inert, `cudnn.benchmark` would thrash because batch sizes vary by design, and `torch.compile`/CUDA graphs do not exist in the pinned torch 1.8. Parallelism across trials is possible via separate processes under CUDA MPS, which sidesteps the process-global RNG hazard that makes `n_jobs > 1` unsafe, but needs per-process seeding and an Optuna RDB backend.
